...
export const Route = createFileRoute("/picks")({
  head: () => ({ meta: [{ title: "Make Picks — Cover League" }] }),
  component: PicksPage,
});
...
function PicksPage() {
  ...
  function toggle(gameId: string, side: PickSide) {
    ...
    setPicks((prev) => {
      ...
      if (!cur && count >= 6) return prev; // limit
    ...
    return (
      <Shell>
        <div className="rounded-2xl border border-border bg-card p-6 text-center">
          ...
          <Link to="/standings" className="mt-4 inline-block">
    ...
    <Shell>
      ...
      <ol className="space-y-3">
        {wkGames.map((g) => {
          ...
          // spread is from HOME's perspective (negative = home favored)
          ...
          return (
            <li key={g.id} className="rounded-xl border border-border bg-card overflow-hidden">
              <div className="px-4 py-2 bg-surface flex items-center justify-between text-[10px] uppercase tracking-widest text-muted-foreground">
                ...
                <span className="font-mono normal-case tracking-normal">{["Sun","Mon","Tue","Wed","Thu","Fri","Sat"][new Date(g.kickoff).getUTCDay()]} {["Jan","Feb","Mar","Apr","May","Jun","Jul","Aug","Sep","Oct","Nov","Dec"][new Date(g.kickoff).getUTCMonth()]} {new Date(g.kickoff).getUTCDate()}</span>
              </div>
...
function SideButton({
  team, city, name, teamId, spread, selected, disabled, onClick,
}: {
