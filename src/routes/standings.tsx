...
export const Route = createFileRoute("/standings")({
  head: () => ({ meta: [{ title: "Standings — Cover League" }] }),
              ...
              return (
                <div key={r.gm.id} className={cn(
                  ...
                  <div className="flex items-center gap-2 min-w-0">
                    <div className="size-7 rounded-full grid place-items-center text-[10px] font-bold text-background shrink-0" style={{ backgroundColor: `oklch(0.75 0.16 ${r.gm.avatarHue})` }}>
...
function WeekView({ weeksAvailable, viewWeek, setViewWeek }: {
  ...
  // Reveal everyone's picks once any game in the week is decided (post-kickoff)
              ...
              <div key={e.gmId} className="rounded-xl border border-border bg-card p-3">
                <div className="flex items-center justify-between mb-2">
                  <div className="flex items-center gap-2">
                    <div className="size-7 rounded-full grid place-items-center text-[10px] font-bold text-background" style={{ backgroundColor: `oklch(0.75 0.16 ${gm.avatarHue})` }}>
...
function MyCard({ entry }: { entry: ReturnType<typeof useLeague>["entries"][number] }) {
