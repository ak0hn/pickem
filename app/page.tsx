...
export default function FeedPage() {
  ...
  return (
    <Shell>
      {!slateLive && (
        <Link href="/commissioner" className="block mb-6">
          <div className="rounded-2xl border border-accent/50 bg-gradient-to-br from-accent/15 via-surface to-surface p-5 hover:border-accent transition-colors">
            ...
            <h2 className="mt-2 text-xl font-display leading-tight">
              {status === 'DRAFT'
                ? `Week ${sim.currentWeek} slate hasn't been pulled yet`
                : `Week ${sim.currentWeek} is in review — publish to open the board`}
            </h2>
            ...
            return (
              <div key={c.id} className="flex gap-2 text-sm">
                <div
                  ...
                  style={{ backgroundColor: `oklch(0.75 0.16 ${gm?.avatarHue ?? 200})` }}
                >
